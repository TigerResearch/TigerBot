...
- 由于各类数据集的质量存在差异，通过Alpaca Self-Instruct生成的数据亦存在质量问题。为解决此问题，我们经过细致的人工校验和分类，总结出一套全面且系统化的数据清洗规则与方法。
- 整体规则可以划分为**过滤类规则**和**清洗类规则**两大类型。其中，命中过滤规则的数据项将被弃用，而清洗规则则旨在处理并保留所需的数据。
- 同时，在我们的数据梳理与积累的过程中，我们也不断对清洗规则进行迭代和优化。
- 通用清洗规则描述如下所示：
 
  a. 过滤-敏感词规则：基于积累的敏感词库，清洗丢弃涉政、涉黄、涉暴、涉恐等数据项；
  
  b. 过滤-无效输入输出： 此规则主要针对Self-Instruct生成数据缺陷进行专项清理，根据输入输出不同分别制定规则进行过滤，以排除一些非正常输出，如"<一段文本>", 无效输出，如"[图画]"；
  
  c. 清洗-关键词规则：根据整理的关键词/正则列表进行数据的替换，包括：过滤特殊无效输入如"<无>"、特殊标识字符清洗、非可见字符清洗、标签类关键词清洗等；
  
  d. 清洗-特殊逻辑规则：此类规则用于清洗sft数据中的一些特殊现象，如指令与输入重复等，如下所示：
  > ```{"instruction": "描述如何做一道红烧肉。请提供食材和详细的步骤。", "input": "请描述如何做一道红烧肉，提供食材和详细步骤。",  "output": ""}```
