...
- 整体规则可以划分为**过滤类规则**和**清洗类规则**两大类。其中，命中过滤规则的数据项将被弃用，而清洗规则则旨在处理并保留所需的数据。
...
- 通用清洗规则描述如下所示：
  ...
  c. 清洗-关键词规则：根据整理的关键词/正则列表进行数据的替换，包括：过滤特殊无效输入如"<无>"、清理特殊标识字符、清理非可见字符、清理标签等；
  
  d. 清洗-特殊逻辑规则：此类规则用于清洗一些特殊现数据象，如指令与输入重复等，如下所示：
